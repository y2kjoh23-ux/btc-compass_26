...
const App: React.FC = () => {
  ...
  useEffect(() => {
    if (data && stats) {
      ...
      if (!lastEntry || (now - lastEntry.id >= FIVE_MINUTES)) {
        ...
        const updated = [newEntry, ...parsed].slice(0, 300);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-300 font-sans text-left relative overflow-x-hidden">
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/95 backdrop-blur-md" onClick={() => setShowHistory(false)}>
          <div className="bg-slate-900 w-full max-w-6xl max-h-[92vh] rounded-[2rem] border border-white/10 flex flex-col overflow-hidden shadow-2xl" onClick={e => e.stopPropagation()}>
            <div className="px-6 py-5 border-b border-white/5 flex justify-between items-center bg-slate-900/50">
              <h3 className="text-base font-black italic uppercase tracking-widest text-white">Neural Snapshot Log (5m Cycle)</h3>
            ...
            <div className="flex-1 overflow-y-auto px-2 py-6 custom-scrollbar">
              {aiAnalysis && <div className="mb-6 bg-indigo-500/5 border border-indigo-500/20 rounded-2xl p-5 text-[13px] leading-relaxed italic text-slate-300">{aiAnalysis.summary}</div>}

              {/* Log Table Header: DEV ($) -> DEV 변경 */}
              ...
              <div className="space-y-1.5 mb-10">
                {history.length === 0 ? <div className="py-24 text-center opacity-20 text-[12px] uppercase font-black tracking-widest">No Logs Found</div> : 
                  history.map((h, idx) => {
                    ...
                    return (
                      <div key={h.id}>
                        {/* 폰트 겹침 방지를 위해 tracking-tighter 및 폰트 사이즈 미세 조정 (text-[10px]) */}
              ...
              {history.length >= 2 && (
                <div className="mt-10 bg-slate-950/50 border border-white/5 rounded-[2rem] p-8">
                  <div className="flex items-center gap-2.5 mb-5">
                    ...
                    <span className="text-[12px] font-black uppercase tracking-[0.25em] text-emerald-500 italic">Intra-Day Flow Synthesis (5m Unit)</span>
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div className="space-y-5">
                      <div className="bg-white/5 p-5 rounded-2xl border border-white/5">
                        <p className="text-[11px] font-black text-slate-600 uppercase mb-3 tracking-widest">단기 가격 추이 (Micro Momentum)</p>
                        <p className="text-[13px] leading-relaxed text-slate-400 italic font-medium">
                          최근 {history.length * 5}분간의 데이터 분석 결과, 가격은 <span className="text-slate-200 font-black">${Math.round(history[history.length-1].price).toLocaleString()}</span>에서 
                          현재 <span className="text-white font-black">${Math.round(history[0].price).toLocaleString()}</span>로 이동했습니다. 
                          5분 단위의 초정밀 스냅샷은 시장의 미세한 심리 변화를 반영하며, 현재는 {history[0].price > history[1].price ? "상방 압력이 우세한 소폭 반등" : "하방 압력이 작용하는 미세 조정"} 구간으로 관측됩니다.
                        </p>
                      </div>
                    </div>
                    <div className="space-y-5">
                      <div className="bg-indigo-500/5 p-5 rounded-2xl border border-indigo-500/10">
                        <p className="text-[11px] font-black text-indigo-400/70 uppercase mb-3 tracking-widest">데이터 시계열 진단 (Time-Series AI)</p>
                        {aiAnalysis ? (
                          <p className="text-[13px] leading-relaxed text-slate-400 italic font-bold">{aiAnalysis.summary}</p>
                        ) : (
                          <p className="text-[13px] leading-relaxed text-slate-400 italic">5분 간격의 미세 변동성을 기반으로 한 AI 분석 결과가 곧 도출됩니다. 잠시만 기다려 주십시오.</p>
                        )}
                      </div>
                    </div>
                  </div>
            ...
            {/* 로그 푸터 폰트 반단계 축소 (text-[9px]) */}
            <div className="p-5 bg-slate-950/50 border-t border-white/5 flex justify-between items-center">
              <span className="text-[9px] font-black text-slate-600 uppercase tracking-widest italic">5M Auto-Sync: DEV(Gap to Fair) | Micro-Flow Synthesis</span>
              <button onClick={clearHistory} className="px-4 py-2 text-[9px] font-black uppercase tracking-widest text-slate-500 hover:text-rose-500 transition-colors">Clear All Logs</button>
            </div>
          </div>
        </div>
      )}
      ...
      <header className="max-w-screen-2xl mx-auto px-4 py-3 flex justify-between items-center border-b border-white/5 bg-slate-950/80 backdrop-blur-md sticky top-0 z-50">
        <h1 className="text-lg font-black text-white tracking-tighter italic uppercase flex items-baseline gap-1.5">BIT COMPASS <span className="text-amber-500">PRO</span> <span className="text-[12px] font-bold text-slate-700 tracking-widest not-italic">v11.6</span></h1>
          ...
          <div className="space-y-10">
            <div className="space-y-4">
              ...
              <div className="flex items-baseline gap-3">
                ...
                {/* 이격도 가격을 원화 가격 폰트와 동일하게 설정 (text-xl) */}
